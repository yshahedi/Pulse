function ReverseVoucher(orgId, req) {
    ...
    try {
        ...
        if (!req?.voucher_ref_ids) throw ('reference vouchers not defined!')
        ...
        let mainVouchers = Sqlite(orgId, db, `SELECT * FROM voucher_tab WHERE  id in (${req?.voucher_ref_ids?.join(',') ?? 0}) AND status_id=2000 AND is_reverse=0 AND business_event_id_fk=${req?.business_event_id}`);
        if (!mainVouchers) throw ('Main voucher not found!');

        mainVouchers?.forEach(mainVoucher => {
            let mainVoucherId = mainVoucher?.id ?? 0;
            ...
            let rec = Sqlite(orgId, db, `SELECT * FROM business_event_posting_type_tab WHERE  id=${mainVoucher?.business_event_posting_type_id_fk};`)?.[0];
    ...
    VALUES
        ...
        ${mainVoucher?.payment_order_id_fk ?? 'null'},
        ${mainVoucher?.financial_order_id_fk ?? 'null'},
        ${mainVoucher?.payment_id_fk ?? 'null'},
        '${data}',
        ${voucherAmount},
        ${voucherCurrencyAmount},
        ${rec.business_event_id_fk ?? 0},
        ${mainVoucher?.transaction_id ?? 'null'}
        );`);
