...
namespace sf {
    ...
    public:
        ...
        void lock() { for (std::atomic<size_t> i = 0; !try_lock(); i++) if (i % 100000 == 0) std::this_thread::yield(); }
    ...
    public:
        ...
        void lock() {
            for (std::atomic<size_t> i = 0; !try_lock(); i++)
                ...
                if (register_index >= 0) {
                    ...
                    if (recursion_depth > 1)
                        ...
                        while (want_x_lock.load(std::memory_order_seq_cst)) {
                            ...
                            for (std::atomic<size_t> i = 0; want_x_lock.load(std::memory_order_seq_cst); i++) 
								if (i % 100000 == 0) std::this_thread::yield();
